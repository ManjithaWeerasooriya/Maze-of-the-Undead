...
public class PathfindingGrid : MonoBehaviour
{
    ...
    [Header("Dynamic Obstacle Blocking")]
    [SerializeField] private float dynamicBlockPadding = 0.5f;

    private Dictionary<GameObject, List<PathNode>> dynamicBlocksByOwner =
        new Dictionary<GameObject, List<PathNode>>();
    ...
    [SerializeField] private float waypointYOffset = 0.3f;

    [Header("Wall Detection")]
    [Tooltip("Nodes within this distance of a NavMesh edge are marked blocked. Use small values like 0.2-0.3 for this level.")]
    public float wallEdgeBuffer = 0.2f;
    ...
    private void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int z = 0; z < gridSizeZ; z++)
            {
                ...
                Vector3 finalPos = onNavMesh
                    ? hit.position + Vector3.up * waypointYOffset
                    : worldPoint;
        ...
        Debug.Log(
            $"[PathfindingGrid] Grid built — Walkable: {walkableCount}/{gridSizeX * gridSizeZ} " +
    ...
    public PathNode NodeFromWorldPoint(Vector3 worldPosition)
    {
        ...
        float percentZ =
            (worldPosition.z - transform.position.z + gridWorldSize.y / 2f) / gridWorldSize.y;
    ...
    public List<PathNode> GetNeighbours(PathNode node)
    {
        ...
        for (int dx = -1; dx <= 1; dx++)
        {
            for (int dz = -1; dz <= 1; dz++)
            {
                ...
                Vector3 navStart = node.worldPosition - Vector3.up * waypointYOffset;
                Vector3 navEnd = neighbour.worldPosition - Vector3.up * waypointYOffset;

                NavMeshHit navHit;
                if (NavMesh.Raycast(navStart, navEnd, out navHit, NavMesh.AllAreas))
                {
                    continue;
                }
    ...
    public int UpdateDynamicObstacle(GameObject owner, Bounds obstacleBounds)
    {
        ...
        List<PathNode> blockedNodes = new List<PathNode>();
        ...
        dynamicBlocksByOwner[owner] = blockedNodes;

        Debug.Log($"[PathfindingGrid] Blocked {blockedNodes.Count} dynamic nodes for {owner.name}.");
    ...
    public List<PathNode> GetNodesInBounds(Bounds bounds)
    {
        ...
        for (int x = 0; x < grid.GetLength(0); x++)
        {
            for (int z = 0; z < grid.GetLength(1); z++)
            {
                ...
                bool insideX = nodePosition.x >= bounds.min.x &&
                               nodePosition.x <= bounds.max.x;

                bool insideZ = nodePosition.z >= bounds.min.z &&
                               nodePosition.z <= bounds.max.z;
